...
function Me.ProduceCurrencyEditor_ProduceCurrency( data )
	...
	for i = 1, #currencies do
		if currencies[i] and currencies[i].guid == data.guid then
			Me.Profile.currency[i].value = Me.Profile.currency[i].value + data.count;
			if Me.Profile.currency[i].value < 0 then
...
function Me.ProduceCurrencyEditor_Save()
	if not UIDropDownMenu_GetText( DiceMasterProduceCurrencyEditor.currencyName ) then
		UIErrorsFrame:AddMessage( "You must select a currency from the dropdown.", 1.0, 0.0, 0.0, 53, 5 );
		return
	end
	local produceData = {
		type = "currency";
		...
		count = DiceMasterProduceCurrencyEditor.currencyCount:GetText();
	}
