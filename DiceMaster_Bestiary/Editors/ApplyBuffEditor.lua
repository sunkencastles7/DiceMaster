...
function Me.DMBuffEditor_Delete()
	if not( Me.dmbuffeditor.collectionName ) then
		return
	end

	local collectionName = Me.dmbuffeditor.collectionName;

	for i = 1, #Me.db.global.collections.buffs[ collectionName ] do
		local buff = Me.db.global.collections.buffs[ collectionName ][i];
		if buff.name == Me.dmbuffeditor.buffName:GetText() and buff.icon == Me.dmbuffeditor.buffIcon.icon:GetTexture() then
			...
			tremove( Me.db.global.collections.buffs[ collectionName ], i )
			break
		end
	end
end

function Me.DMBuffEditor_Save( collectionName )
	...
	for k, v in pairs( Me.db.global.collections.buffs ) do
		for i = 1, #v do
			if v[i].name == buff.name then
				UIErrorsFrame:AddMessage( "You have already saved a buff with that name in this collection.", 1.0, 0.0, 0.0 );
				break
			elseif i == #v then
				tinsert( v, buff );
				return
...
--
function Me.DMBuffEditorLoadDropDown_OnClick(self, arg1, arg2, checked)
	...
	if arg2 then
		editor.collectionName = arg2;
	end
end

function Me.DMBuffEditorLoadDropDown_OnLoad( frame, level, menuList )
	...
	if level == 1 then
		...
		info.hasArrow = false;
		info.menuList = nil;
	elseif menuList == "Conditions" then
		for i = 1,#Me.TermsList["Conditions"] do
			...
			info.func = Me.DMBuffEditorLoadDropDown_OnClick;
			UIDropDownMenu_AddButton(info, level); 
		end
	elseif menuList then
		for i = 1, #Me.db.global.collections.buffs[menuList] do
			local buff = Me.db.global.collections.buffs[menuList][i];
			if not buff then 
				return 
			end
...
-------------------------------------------------------------------------------
-- Dropdown handlers for the save buffs menu.
--
function Me.DMBuffEditorSaveDropDown_OnClick(self, arg1, arg2, checked)
	Me.DMBuffEditor_Save( arg1 );
end

function Me.DMBuffEditorSaveDropDown_OnLoad( frame, level, menuList )
	local info      = UIDropDownMenu_CreateInfo();
	for k, v in pairs( Me.db.global.collections.buffs ) do
		info.tooltipTitle = k;
		info.tooltipText = "Click to save this buff to this collection.";
		info.tooltipOnButton = true;
		info.text       = k;
		info.notCheckable = true;
		info.arg1		= k;
		info.func       = Me.DMBuffEditorSaveDropDown_OnClick;
		UIDropDownMenu_AddButton(info); 
	end
end
