...
StaticPopupDialogs["DICEMASTER4_CAGEPETITEM"] = {
  text = "Put this pet in a cage?",
  button1 = "Request",
  button2 = "Cancel",
  OnAccept = function ( self, data )
	local item = Me.Profile.pet.item
	if not item then
		return
	end

	-- TODO copy pet attributes and create an item
...
local function OnPetClick( self, button )
	local item = Me.db.global.bank[self.itemIndex]
	local cursorIcon = DiceMasterCursorItemIcon
	
	if ( button == "RightButton" ) then
		StaticPopup_Show( "DICEMASTER4_CAGEPETITEM" )
	end
end
...
local methods = {
	...
	---------------------------------------------------------------------------
	-- Hook this button up to a pet item.
	--
	SetPetItem = function( self )
		self.item = nil;
		self.itemIndex = nil;
		self.itemPlayer = nil;
		self.itemPet = true;
		
		self:SetScript( "OnClick", OnPetClick )
	...
	Update = function( self )
		...
		elseif self.itemPet then
			item = Me.Profile.pet.item
		else
